...
def test_place_search_falls_back_to_baidu_when_amap_quota_exceeded(monkeypatch) -> None:
    ...
    assert "/place/v2/search" in calls[0][0]
    assert calls[0][1]["query"] == "quietproposalplace"
    assert calls[0][1]["region"] == "Hangzhou"
    assert calls[0][1]["tag"] == "nightview,quiet"
    assert calls[0][1]["page_size"] == 10
    assert calls[0][1]["ret_coordtype"] == "gcj02ll"
    assert calls[0][1]["city_limit"] == "false"
