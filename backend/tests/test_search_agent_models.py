...
def test_openai_compatible_extract_sends_chat_completion_request(monkeypatch) -> None:
    ...
    result = OpenAICompatibleHttpAgentModel().extract(
...
def test_openai_compatible_extract_uses_context_builder_history_without_adapter_slice(monkeypatch) -> None:
    _set_openai_compatible(monkeypatch)
    monkeypatch.setattr(settings, "ai_search_max_history_messages", 2)
    captured = {}

    def fake_post(url, *, headers, json, timeout):
        del url, headers, timeout
        captured["messages"] = json["messages"]
        content = {"intent": "continue", "assistant_message": "What date?"}
        return _FakeResponse({"choices": [{"message": {"content": json_dumps(content)}}]})

    monkeypatch.setattr("app.agents.search_agent_models.httpx.post", fake_post)
    conversation = [
        AIChatMessage(role="user", content=f"user history {index}")
    ...
    OpenAICompatibleHttpAgentModel().extract(
        ...
        city_catalog="Beijing(BJ/Beijing)",
    )
    ...
    assert "user history 0" in user_contents
    assert "user history 4" in user_contents
    assert user_contents[-1] == "latest"


def test_extraction_prompt_keeps_cacheable_prefix_stable(monkeypatch) -> None:
    monkeypatch.setattr(settings, "ai_agent_prompt_cache_optimization_enabled", True)
    first = _extraction_prompt_parts(
        language="en",
        parsed_request=ParsedSearchRequest(from_city="Beijing"),
        city_catalog="Beijing(BJ/Beijing), Shanghai(SH/Shanghai)",
    )
    second = _extraction_prompt_parts(
        language="zh",
        parsed_request=ParsedSearchRequest(from_city="Shanghai", to_city="Chengdu"),
        city_catalog="Beijing(BJ/Beijing), Shanghai(SH/Shanghai), Chengdu(CD/Chengdu)",
    )
...
def test_openai_compatible_reply_does_not_force_json_mode(monkeypatch) -> None:
    ...
    def fake_post(url, *, headers, json, timeout):
        ...
        return _FakeResponse({
            "choices": [{"message": {"content": "What date do you want to travel?"}}],
            "usage": {
                "prompt_tokens": 100,
    ...
    result = OpenAICompatibleHttpAgentModel().reply(
        language="en",
    ...
    assert result.value == "What date do you want to travel?"
    assert result.usage is not None
    ...
    assert result.usage.cache_hit_ratio == 0.4
...
def test_openai_compatible_marks_context_length_errors(monkeypatch) -> None:
    _set_openai_compatible(monkeypatch)

    def fake_post(url, *, headers, json, timeout):
        del headers, json, timeout
        request = httpx.Request("POST", url)
        response = httpx.Response(400, request=request, text="maximum context length exceeded")
        raise httpx.HTTPStatusError("bad request", request=request, response=response)

    monkeypatch.setattr("app.agents.search_agent_models.httpx.post", fake_post)

    with pytest.raises(AIClientError, match="context length"):
        OpenAICompatibleHttpAgentModel().reply(
            language="en",
            parsed_request=ParsedSearchRequest(from_city="Beijing", to_city="Shanghai"),
            missing_fields=["travel_date"],
            next_question_field=None,
